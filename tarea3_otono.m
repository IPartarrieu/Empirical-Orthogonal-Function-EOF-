...
clear ISO T2
%% sacamos promedios para los meses de otoño MAM
a=1;
for i=3:12:756
    iso_anom_oto(:,:,a)=mean(iso_anom(:,:,i:i+2),3);
    t2_anom_oto(:,:,a)=mean(t2_anom(:,:,i:i+2),3);
    a=a+1;
end
% tenemos que trabajar con los datos sin tendencia
iso_anom_oto_dt=detrend3(iso_anom_oto);
t2_anom_oto_dt=detrend3(t2_anom_oto);
%
% determinamos X Y y T para trabajar
[X,Y,T]=size(iso_anom_oto_dt);
%
%
%
%
%% EOF combinada, caso hacia abajo
S = reshape(permute(iso_anom_oto_dt,[3 1 2]),T,X*Y);
P = reshape(permute(t2_anom_oto_dt,[3 1 2]),T,X*Y);
F=[S';P'];
...
corr_iso=corr3(iso_anom_oto_dt,A1(:,1)/std(A1(:,1)));
corr_t2=corr3(t2_anom_oto_dt,A1(:,1)/std(A1(:,1)));
...
ylabel('Latitud','FontSize',14)
xlabel('Longitud','fontsize',14)
sgtitle(['Otoño - P.V: ',num2str(var),'%'],'fontsize',20)
