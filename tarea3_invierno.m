...
clear ISO T2
%% sacamos promedios para los meses de invierno JJA
a=1;
for i=6:12:756
    iso_anom_inv(:,:,a)=mean(iso_anom(:,:,i:i+2),3);
    t2_anom_inv(:,:,a)=mean(t2_anom(:,:,i:i+2),3);
    a=a+1;
end
% tenemos que trabajar con los datos sin tendencia
iso_anom_inv_dt=detrend3(iso_anom_inv);
t2_anom_inv_dt=detrend3(t2_anom_inv);
%
% determinamos X Y y T para trabajar
[X,Y,T]=size(iso_anom_inv_dt);
%
%
%
%
%% EOF combinada, caso hacia abajo
S = reshape(permute(iso_anom_inv_dt,[3 1 2]),T,X*Y);
P = reshape(permute(t2_anom_inv_dt,[3 1 2]),T,X*Y);
F=[S';P'];
...
corr_iso=corr3(iso_anom_inv_dt,A1(:,1)/std(A1(:,1)));
corr_t2=corr3(t2_anom_inv_dt,A1(:,1)/std(A1(:,1)));
...
ylabel('Latitud','FontSize',14)
xlabel('Longitud','fontsize',14)
sgtitle(['Invierno - P.V: ',num2str(var),'%'],'fontsize',20)
